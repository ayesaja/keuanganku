...
const PARENT_CATS = [
  { id:"makan-minum", label:"Makan & Minum", color:"#F59E0B", bg:"#FEF9EE",
    subcats:[{id:"makan-dadakan",label:"Makan dadakan"},{id:"jajan-anak",label:"Jajan anak"},{id:"kopi",label:"Kopi"},{id:"susu",label:"Susu"}]},
  { id:"tagihan", label:"Tagihan", color:"#EF4444", bg:"#FEF2F2",
    subcats:[{id:"internet",label:"Internet"},{id:"telepon",label:"Telepon"},{id:"listrik",label:"Listrik"},{id:"air",label:"Langganan air"}]},
  { id:"transportasi", label:"Transportasi", color:"#3B82F6", bg:"#EFF6FF",
    subcats:[{id:"tol",label:"Biaya tol"},{id:"bensin",label:"Bensin"},{id:"parkir",label:"Parkir"},{id:"perawatan-kend",label:"Perawatan kendaraan"},{id:"taxi",label:"Taxi"}]},
  { id:"belanja", label:"Belanja", color:"#8B5CF6", bg:"#F5F3FF",
    subcats:[{id:"pakaian",label:"Pakaian"},{id:"perlengkapan",label:"Perlengkapan sehari-hari"},{id:"perabot-rumah",label:"Perabot rumah"},{id:"pakan-kucing",label:"Pakan kucing"},{id:"keperluan-dapur",label:"Keperluan dapur"}]},
  { id:"bepergian", label:"Biaya Bepergian", color:"#06B6D4", bg:"#ECFEFF",
    subcats:[{id:"akomodasi-lk",label:"Akomodasi luar kota"},{id:"sewa-hotel",label:"Sewa hotel"},{id:"tiket-antarkota",label:"Tiket transportasi antar kota"}]},
  { id:"hadiah-donasi", label:"Hadiah & Donasi", color:"#EC4899", bg:"#FDF2F8",
    subcats:[{id:"hadiah-teman",label:"Hadiah teman"},{id:"sedekah",label:"Sedekah"}]},
  { id:"pendidikan", label:"Pendidikan", color:"#F97316", bg:"#FFF7ED",
    subcats:[{id:"buku",label:"Buku"},{id:"media-belajar",label:"Media belajar anak"},{id:"spp",label:"SPP Bulanan"},{id:"biaya-tahunan",label:"Biaya pendidikan tahunan"},{id:"kursus",label:"Biaya kursus"}]},
  { id:"kesehatan", label:"Biaya Kesehatan", color:"#10B981", bg:"#F0FDF9",
    subcats:[{id:"dokter",label:"Jasa dokter"},{id:"obat",label:"Obat-obatan"}]},
  { id:"bisnis", label:"Bisnis / Pekerjaan", color:"#6366F1", bg:"#EEF2FF",
    subcats:[{id:"langganan-digital",label:"Langganan layanan digital"},{id:"akomodasi-dinas",label:"Akomodasi perjalanan dinas"},{id:"cetak-media",label:"Cetak media ajar"}]},
];
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      newTxs=(obj.transactions||[]).map(t=>({
        ...
        merchant:t.merchant||"-",amount:Number(t.amount)||0,
        category:t.category||"makan-minum",subcategory:t.subcategory||"makan-dadakan",
        date:t.date||todayStr(),items:t.items||"",createdAt:new Date().toISOString(),
      }));
      actions=obj.actions||[];
    } catch(e) {
    ...
    for(const a of actions){
      if(a.type==="update_category"){
        const kw=(a.keyword||"").toLowerCase(); let count=0;
        ...
        if(count>0){const sub=getSubcat(a.new_subcategory);actionLog.push(`✓ ${count} transaksi "${a.keyword}" → ${sub?.parent.label} › ${sub?.label}`);}
      }
    }
    saveTx(updatedTx);
            ...
            {panel==="tx"&&(
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                  ...
                  <select value={txMonth} onChange={e=>setTxMonth(e.target.value)} style={{fontSize:12,padding:"5px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#F9FAFB",color:"#374151",cursor:"pointer"}}>
                  ...
                  :panelTxs.map(t=>{
                    ...
                    return(
                      <div key={t.id} style={{marginBottom:6}}>
                        <div onClick={()=>setExpandedTx(isOpen?null:t.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:isOpen?"#F0FDF9":"#F8FAFC",borderRadius:isOpen?"10px 10px 0 0":10,border:"0.5px solid #E5E7EB",cursor:"pointer"}}>
                          <div style={{width:9,height:9,borderRadius:"50%",background:par.color,flexShrink:0}}/>
                          <div style={{flex:1,minWidth:0}}>
                        ...
                        {isOpen&&(
                          <div style={{background:"#F9FAFB",border:"0.5px solid #E5E7EB",borderTop:"none",borderRadius:"0 0 10px 10px",padding:"12px 12px 10px"}}>
                            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                              <div><div style={{fontSize:11,color:"#6B7280",fontWeight:500,marginBottom:4}}>Merchant</div><input defaultValue={t.merchant} onBlur={e=>updateTx(t.id,{merchant:e.target.value})} style={{width:"100%",fontSize:12,padding:"6px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#fff",color:"#111827",boxSizing:"border-box",outline:"none"}}/></div>
                              <div><div style={{fontSize:11,color:"#6B7280",fontWeight:500,marginBottom:4}}>Jumlah (Rp)</div><input type="number" defaultValue={t.amount} onBlur={e=>updateTx(t.id,{amount:parseInt(e.target.value)||0})} style={{width:"100%",fontSize:12,padding:"6px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#fff",color:"#111827",boxSizing:"border-box",fontFamily:"'DM Mono',monospace",outline:"none"}}/></div>
                            </div>
                            <div style={{marginBottom:8}}>
                              <div style={{fontSize:11,color:"#6B7280",fontWeight:500,marginBottom:4}}>Kategori › Subkategori</div>
                              <select value={catVal(t)} onChange={e=>{const p=parseCatVal(e.target.value);updateTx(t.id,p);}} style={{width:"100%",fontSize:12,padding:"6px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#fff",color:"#111827",cursor:"pointer",boxSizing:"border-box",outline:"none"}}>
                                {PARENT_CATS.map(p=><optgroup key={p.id} label={p.label}>{p.subcats.map(s=><option key={s.id} value={`${p.id}||${s.id}`}>{s.label}</option>)}</optgroup>)}
                              </select>
                            </div>
                            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                              <div><div style={{fontSize:11,color:"#6B7280",fontWeight:500,marginBottom:4}}>Tanggal</div><input type="date" defaultValue={t.date} onBlur={e=>updateTx(t.id,{date:e.target.value})} style={{width:"100%",fontSize:12,padding:"6px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#fff",color:"#111827",boxSizing:"border-box",outline:"none"}}/></div>
                              <div><div style={{fontSize:11,color:"#6B7280",fontWeight:500,marginBottom:4}}>Keterangan</div><input defaultValue={t.items} onBlur={e=>updateTx(t.id,{items:e.target.value})} style={{width:"100%",fontSize:12,padding:"6px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#fff",color:"#111827",boxSizing:"border-box",outline:"none"}}/></div>
                            </div>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                              ...
                              <button onClick={()=>{if(window.confirm("Hapus transaksi ini?")){deleteTx(t.id);setExpandedTx(null);}}} style={{fontSize:12,padding:"6px 10px",background:"transparent",color:"#EF4444",border:"0.5px solid #FCA5A5",borderRadius:7,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}><Trash2 size={12}/> Hapus</button>
                            </div>
                          </div>
                        )}
                      </div>
            ...
            {panel==="rekap"&&(
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                  ...
                  <select value={selMonth} onChange={e=>setSelMonth(e.target.value)} style={{fontSize:12,padding:"5px 8px",borderRadius:7,border:"0.5px solid #E5E7EB",background:"#F9FAFB",color:"#374151",cursor:"pointer"}}>
                    {allMonths().map(m=><option key={m} value={m}>{prettyM(m)}</option>)}
                  </select>
                </div>
                  ...
                  <>
                    <div style={{height:150,marginBottom:14}}>
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart><Pie data={parentData} dataKey="total" cx="50%" cy="50%" innerRadius={40} outerRadius={62} stroke="none">{parentData.map(p=><Cell key={p.id} fill={p.color}/>)}</Pie><Tooltip formatter={v=>fmtRp(v)} contentStyle={{fontSize:12,borderRadius:8,border:"0.5px solid #E5E7EB"}}/></PieChart>
                      </ResponsiveContainer>
                    </div>
                    {[...parentData].sort((a,b)=>b.total-a.total).map(p=>{
                      const subBreak=p.subcats.map(s=>({...s,total:monthTx.filter(t=>t.subcategory===s.id).reduce((sum,t)=>sum+(t.amount||0),0)})).filter(s=>s.total>0).sort((a,b)=>b.total-a.total);
                      return(
                        <div key={p.id} style={{marginBottom:12}}>
                          ...
                          {subBreak.map(s=><div key={s.id} style={{display:"flex",alignItems:"center",gap:6,paddingLeft:18,marginBottom:2}}><div style={{width:5,height:5,borderRadius:"50%",background:p.color,opacity:0.5}}/><span style={{fontSize:11,color:"#6B7280",flex:1}}>{s.label}</span><span style={{fontSize:11,fontFamily:"'DM Mono',monospace",color:"#374151"}}>{fmtRp(s.total)}</span></div>)}
                        </div>
                      );
                    })}
                  </>
